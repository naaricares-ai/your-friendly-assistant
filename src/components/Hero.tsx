    ...
    >
      ...
      >
        ...
        <div className="text-center lg:text-left">
          ...
          <h1 className="mb-8 perspective-1000">
            <div className="text-hero text-5xl md:text-6xl lg:text-7xl text-white overflow-hidden">
              <AnimatedText text={hero.title1} delay={0} />
            </div>
            <div className="text-hero text-5xl md:text-6xl lg:text-7xl text-white overflow-hidden">
              <AnimatedText text={hero.title2} delay={0.3} />
            </div>
            <div className="text-hero-alt text-5xl md:text-6xl lg:text-8xl gradient-text-premium overflow-hidden mt-2">
              <AnimatedText text={hero.title3} delay={0.6} />
            </div>
            <div className="text-elegant text-4xl md:text-5xl lg:text-6xl text-electric-blue overflow-hidden mt-1">
          ...
          <motion.p
            ...
            className="font-body text-lg md:text-xl text-titanium leading-relaxed mb-10 max-w-xl mx-auto lg:mx-0"
          >
            {hero.subtitle}
          </motion.p>
          ...
          >
            <MagneticButton
              href="#solutions"
              className="btn-premium px-8 py-4 rounded-full text-white flex items-center justify-center gap-3 group"
            >
              <span className="relative z-10 font-body font-medium">{hero.ctaPrimary}</span>
              <ArrowRight className="w-5 h-5 relative z-10 group-hover:translate-x-1 transition-transform" />
            </MagneticButton>
            <MagneticButton
              href="#showcase"
              className="btn-ghost px-8 py-4 rounded-full text-white flex items-center justify-center gap-3"
            >
              <Play className="w-5 h-5" />
              <span className="font-body font-medium">{hero.ctaSecondary}</span>
            </MagneticButton>
          </motion.div>
          
          {/* Stats with Premium Typography */}
          <motion.div
            initial={{ opacity: 0, y: 30 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 1, delay: 1.6 }}
            className="grid grid-cols-3 gap-8 mt-16 pt-10 border-t border-white/5"
          >
            {hero.stats.map((stat, index) => (
              ...
              >
                <div className="text-number text-4xl md:text-5xl font-bold text-white mb-1">
                  {stat.value}
                </div>
      ...
      {/* Bottom Marquee */}
      <div className="absolute bottom-0 left-0 right-0 overflow-hidden py-8 border-t border-white/5">
        <Marquee 
